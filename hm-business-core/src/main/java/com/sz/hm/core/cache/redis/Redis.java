...
public class Redis {
	...
	public Redis(String masterName, Set<String> sentinels, int database) {
		pool = new JedisSentinelPool(masterName, sentinels, new GenericObjectPoolConfig(), DEFAULT_TIMEOUT, null, database);
		ping();
		logger.info("Redis 连接信息:sentines={},database={},timeout={}ms,password={}", JSON.toJSONString(sentinels), database, DEFAULT_TIMEOUT, null);
	}
	...
	public Redis(String host, int port, int timeout, String password, int index) {
		// 通过config配置连接池参数
		this(new GenericObjectPoolConfig(), host, port, DEFAULT_TIMEOUT, null, index);
	}
	...
	public void shutdown(){
		logger.error("关闭Jedis sentinel线程");
		System.out.println("关闭Jedis sentinel线程");
		if( pool != null ){
			pool.destroy();
		}
	}
